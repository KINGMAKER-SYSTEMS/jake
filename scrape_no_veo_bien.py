"""
Scraper for No Veo Bien by RØZ Campaign
Sound URL: https://www.tiktok.com/music/No-veo-bien-by-R%C3%98Z-and-Nsqk-7575718941533588232
...
sound_url = "https://www.tiktok.com/music/No-veo-bien-by-R%C3%98Z-and-Nsqk-7575718941533588232"
# Extract the sound ID directly from the URL
import re
match = re.search(r'-(\d+)$', sound_url)
if match:
    sound_id = match.group(1)
    sound_title = "No veo bien"
...
if all_matching_videos:
    output_file = f"output/no_veo_bien_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
