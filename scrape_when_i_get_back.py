...
reference_url = "https://www.tiktok.com/@minjeong1116/video/7580318226282745106"

print("=" * 80)
print("OPTIMIZED SOUND CAMPAIGN SCRAPER - 'WHEN I GET BACK ON MY FEET'")
print("=" * 80)
...
if not sound_id:
    ...
    sound_id = "7415814909060040721"
    sound_title = "When I Get Back On My Feet"
...
for account in accounts:
    ...
    if cached_videos:
        print(f"  [OK] Loaded {len(cached_videos)} videos from cache (last scrape: {last_scrape})")
        ...
        print(f"  [OK] {len(cached_with_sound_ids)} videos already have sound IDs extracted")
        ...
        print(f"  [OK] {len(cached_videos_in_range)} cached videos since {start_date.strftime('%b %d, %Y')}")
    else:
        print(f"  [i] No cache found, will scrape all videos")
        cached_videos = []
        cached_videos_in_range = []
    ...
    if videos_needing_ids:
        ...
        print(f"  [OK] Saved updated cache")
    else:
        print(f"  [OK] All videos already have sound IDs (using cache)")

    # Filter for matching sound
    matching = [v for v in all_videos if str(v.get('extracted_sound_id')) == str(sound_id)]

    if matching:
        print(f"  [OK] Found {len(matching)} videos with matching sound")
        # Show top 3
        ...
        print(f"  [X] No videos with matching sound")
