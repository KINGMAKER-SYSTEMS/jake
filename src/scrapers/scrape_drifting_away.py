#!/usr/bin/env python3
"""
Scrape specific accounts for "Drifting Away" by Mattilo
"""
...
def main():
    # Accounts from updated campaign CSV
    accounts = [
        'sxalenz',
        'enzowms',
        'enzorealasf',
    ...
    start_date = datetime(2025, 11, 11, 0, 0)  # November 11, 2025
    ...
    all_videos = []
    for account in accounts:
        # Use higher limit to catch more videos, especially for active accounts
        # Also check for the specific video ID if it's @oejee
        videos = scrape_account_videos(account, start_datetime=start_date, end_datetime=end_date, limit=2000)
        
        # Debug: Check if specific video is in results for @oejee
        if account == 'oejee':
            target_video_id = "7580028745411513622"
            for v in videos:
                if target_video_id in v.get('url', ''):
                    print(f"\n  [DEBUG] Found target video in @oejee results!")
                    print(f"    URL: {v.get('url')}")
                    print(f"    Song: '{v.get('song', 'N/A')}'")
                    print(f"    Artist: '{v.get('artist', 'N/A')}'")
                    print(f"    Date: {v.get('timestamp', 'N/A')}")
                    matches = matches_song(v, target_song, target_artist)
                    print(f"    Matches filter: {matches}")
    ...
    # Save to file
    output_file = Path('output') / 'drifting_away_scrape_results.txt'
    output_file.parent.mkdir(exist_ok=True)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"DRIFTING AWAY - MATTILO SCRAPE RESULTS\n")
        f.write("=" * 80 + "\n\n")
