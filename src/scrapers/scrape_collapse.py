#!/usr/bin/env python3
"""
Scrape accounts from CSV for "Collapse" by Chance Peña
"""
import sys
import csv
import re
...
from utils.get_post_links_by_song import scrape_account_videos, get_profile_username, build_profile_url
    ...
    try:
        ...
        for line in result.stdout.strip().split('\n'):
            ...
            try:
                ...
                # Extract sound ID from video URL or metadata
                ...
                if video_dt:
                    ...
                    # If we can't determine date, skip if date filtering is required
                    skipped_old += 1
                    continue
                ...
                try:
                    ...
                    if vid_result.returncode == 0:
                        ...
                        # Check description or other text fields for sound links
                        if not sound_id_match:
                    ...
                    # Silently continue if individual video fetch fails
                    pass
...
def main():
    ...
    csv_path = Path(r"C:\Users\jakeb\OneDrive\Desktop\Claude Sandbox\2025 Sound Campaigns - ☑️ Chance Peña - Collapse.csv")
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            account_url = row['Creator Handles'].strip()  # Strip whitespace
    ...
    print("=" * 80)
    print("SCRAPING 'COLLAPSE' CAMPAIGN")
    print("=" * 80)
    ...
    start_date = datetime(2025, 12, 12, 0, 0)
    ...
    if matching_videos:
        ...
        # Save results
        output_file = Path(__file__).parent.parent.parent / 'output' / 'Collapse_scrape_results.txt'
        copy_paste_file = Path(__file__).parent.parent.parent / 'output' / 'Collapse_copy_paste.txt'
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"COLLAPSE - SCRAPE RESULTS\n")
            f.write("=" * 80 + "\n\n")
        ...
        with open(copy_paste_file, 'w', encoding='utf-8') as f:
            f.write(f"COLLAPSE - COPY/PASTE FORMAT\n")
            f.write("=" * 80 + "\n\n")
            for account, _ in sorted_accounts:
                videos = sorted(by_account[account], key=lambda x: x['views'], reverse=True)
                for video in videos:
