#!/usr/bin/env python3
"""
Scrape accounts from CSV for "Where's Your Head At" by Eurotrip
"""
import sys
import csv
...
def matches_song(video, song_name, artist_name):
    ...
    # For Where's Your Head At, it might be "Where's Your Head At by Eurotrip" or variations
    song_match = "where's your head at" in video_song or "wheres your head at" in video_song or "where your head at" in video_song
    # Artist might be "eurotrip" or "euro trip" or variations
    artist_match = 'euro' in video_artist and 'trip' in video_artist
    return song_match and artist_match

def main():
    # Read accounts from CSV
    csv_path = Path(r"C:\Users\jakeb\OneDrive\Desktop\Claude Sandbox\2025 Sound Campaigns - ☑️ Eurotrip - Where's Your Head At.csv")
    
    accounts = []
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            account_url = row['Creator Handles']
    ...
    print("=" * 80)
    print("SCRAPING 'WHERE'S YOUR HEAD AT' CAMPAIGN")
    print("=" * 80)
    ...
    target_song = "Where's Your Head At"
    target_artist = 'Eurotrip'
    ...
    if matching_videos:
        ...
        # Save results
        output_file = Path(__file__).parent.parent.parent / 'output' / 'Wheres_Your_Head_At_scrape_results.txt'
        copy_paste_file = Path(__file__).parent.parent.parent / 'output' / 'Wheres_Your_Head_At_copy_paste.txt'
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"WHERE'S YOUR HEAD AT - SCRAPE RESULTS\n")
            f.write("=" * 80 + "\n\n")
        ...
        with open(copy_paste_file, 'w', encoding='utf-8') as f:
            f.write(f"WHERE'S YOUR HEAD AT - COPY/PASTE FORMAT\n")
            f.write("=" * 80 + "\n\n")
            for account, _ in sorted_accounts:
                videos = sorted(by_account[account], key=lambda x: x['views'], reverse=True)
                for video in videos:
