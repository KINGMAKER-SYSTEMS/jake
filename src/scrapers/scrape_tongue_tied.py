#!/usr/bin/env python3
"""
Scrape accounts from CSV for "Tongue Tied" by Chance Peña
"""
import sys
import csv
...
def matches_song(video, song_name, artist_name):
    ...
    # For Tongue Tied, it might be "Tongue Tied by Chance Peña" or just "Tongue Tied"
    song_match = 'tongue tied' in video_song
    # Artist might be "chance peña" or "chancepena" or "chance pena"
    artist_match = 'chance' in video_artist and ('peña' in video_artist or 'pena' in video_artist or 'peña' in video_song or 'pena' in video_song)
    return song_match and artist_match

def main():
    # Read accounts from CSV
    csv_path = Path(__file__).parent.parent.parent / 'output' / 'Tongue_Tied_Chance_Peña_campaign.csv'
    ...
    print("=" * 80)
    print("SCRAPING 'TONGUE TIED' CAMPAIGN")
    print("=" * 80)
    ...
    target_song = 'Tongue Tied'
    target_artist = 'Chance Peña'
    ...
    if matching_videos:
        ...
        # Save results
        output_file = Path(__file__).parent.parent.parent / 'output' / 'Tongue_Tied_scrape_results.txt'
        copy_paste_file = Path(__file__).parent.parent.parent / 'output' / 'Tongue_Tied_copy_paste.txt'
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"TONGUE TIED - SCRAPE RESULTS\n")
            f.write("=" * 80 + "\n\n")
        ...
        with open(copy_paste_file, 'w', encoding='utf-8') as f:
            f.write(f"TONGUE TIED - COPY/PASTE FORMAT\n")
            f.write("=" * 80 + "\n\n")
            for account, _ in sorted_accounts:
                videos = sorted(by_account[account], key=lambda x: x['views'], reverse=True)
                for video in videos:
