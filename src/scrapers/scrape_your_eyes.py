#!/usr/bin/env python3
"""
Scrape accounts from CSV for "Your Eyes" by Night Tales
"""
import sys
import csv
...
def matches_song(video, song_name, artist_name):
    ...
    # For Your Eyes, it might be "Your Eyes by Night Tales" or just "Your Eyes"
    song_match = 'your eyes' in video_song
    # Artist might be "night tales" or "nighttales" or variations
    artist_match = 'night' in video_artist and 'tale' in video_artist
    return song_match and artist_match

def main():
    # Read accounts from CSV
    csv_path = Path(r'C:\Users\jakeb\OneDrive\Desktop\Claude Sandbox\2025 Sound Campaigns - ☑️ Night Tales - Your Eyes.csv')
    ...
    print("=" * 80)
    print("SCRAPING 'YOUR EYES' CAMPAIGN")
    print("=" * 80)
    ...
    target_song = 'Your Eyes'
    target_artist = 'Night Tales'
    ...
    if matching_videos:
        ...
        # Save results
        output_file = Path(__file__).parent.parent.parent / 'output' / 'Your_Eyes_scrape_results.txt'
        copy_paste_file = Path(__file__).parent.parent.parent / 'output' / 'Your_Eyes_copy_paste.txt'
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"YOUR EYES - SCRAPE RESULTS\n")
            f.write("=" * 80 + "\n\n")
        ...
        with open(copy_paste_file, 'w', encoding='utf-8') as f:
            f.write(f"YOUR EYES - COPY/PASTE FORMAT\n")
            f.write("=" * 80 + "\n\n")
            for account, _ in sorted_accounts:
                videos = sorted(by_account[account], key=lambda x: x['views'], reverse=True)
                for video in videos:
