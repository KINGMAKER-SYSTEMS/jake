#!/usr/bin/env python3
"""
Scrape specific accounts for "Control" by Aaron Taos
"""
...
def main():
    ...
    start_date = datetime(2025, 11, 7, 0, 0)  # November 7, 2025
    end_date = datetime.now()
    
    # Song to search for
    target_song = 'Control'
    target_artist = 'Aaron Taos'
    ...
    # Save to file
    output_file = Path('output') / 'control_aaron_taos_scrape_results.txt'
    output_file.parent.mkdir(exist_ok=True)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"CONTROL - AARON TAOS SCRAPE RESULTS\n")
        f.write("=" * 80 + "\n\n")
    ...
    copy_paste_file = Path('output') / 'Control_-_Aaron_Taos_copy_paste.txt'
    with open(copy_paste_file, 'w', encoding='utf-8') as f:
        # Sort all videos by views descending
        all_sorted = sorted(matched_videos, key=lambda x: x['views'], reverse=True)
        for video in all_sorted:
            f.write(f"{video['url']}\n")
