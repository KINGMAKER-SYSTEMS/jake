"""
Scraper for Where's Your Head At? by eurotripp Campaign - Round 2
Sound URL: https://www.tiktok.com/music/Where's-Your-Head-At-7507662813536897040
...
sound_url = "https://www.tiktok.com/music/Where's-Your-Head-At-7507662813536897040"
# Extract the sound ID directly from the URL
import re
match = re.search(r'-(\d+)$', sound_url)
if match:
    sound_id = match.group(1)
    sound_title = "Where's Your Head At?"
...
if all_matching_videos:
    output_file = f"output/wheres_your_head_at_round2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
